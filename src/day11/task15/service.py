...
'''
    ...
    1. url 확인
    http://www.qooqoo.co.kr/bbs/board.php?bo_table=storeship&page={page}


    2. 정보의 html 식별자 확인
        1. 정보가 있는 위치 <tbody> 여러개 매장 정보
        2. <tr> 하나의 매장 정보 # 홀수 또는 열 6개 : pc # 짝수 또는  : 모바일
        3. <td> 하나의 매장의 각 속성 1.번호 2.지점 3.연락처 4.주소 5.영업시간
        4. 데이터 상세 위치 확인
            - 번호 <td>
            - 지점 <td> -> <div> -> <a> 2번째
...
def qooqooStoreInfo(result):
    for page in range(1 , 7):
        ...
        if response.getcode() == 200:
            ...
            for row in rows:  # 4-3 행(매장) 마다 파싱
                ...
                # 각 정보들 파싱 , .strip() 사용하여 공백제거
                번호 = cols[0].string.strip()
                지점명 = cols[1].select('a')[1].string.strip() # 2번째 열의 2개 a가 존재하는데 2번째 a태그의 텍스트 파싱
                연락처 = cols[2].select_one('a').string.strip()
                주소 = cols[3].select_one('a').string.strip()
                영업시간 = cols[4].select_one('a').string.strip()

                # 5. 리스트에 담기( 왜? df 사용하기 위해서 2차원 리스트 구성할 예정 )
    ...
    # 7. 리스트 반환
    return result
# [2] 2차원 리스트를 csv 변환 해주는 서비스 , 데이터 , csv 파일명 , 열(제목) 목록
...
# [3] csv 파일을 읽어와서 JSON 형식으로 가져오기 PY 타입으로 가져오기 , 가져올 파일명
def read_csv_to_json( fileName ):
    ...
    df = pd.read_csv(f'{fileName}.csv' , encoding='utf-8',engine='python' , index_col=0)
        # index_col=0 : 판다스의 데이터프레임워크 형식 유지( 테이블 형식 )
...
#     result = []
#     qooqooStoreInfo(result); print(result) # 쿠우쿠우 매장 정보 크롤링 서비스 실행
#     list2d_to_csv(result , '전국쿠우쿠우매장' , ['번호','지점명','연락처','주소','영업시간' ]) # 매장정보 csv로 저장 서비스 호출
