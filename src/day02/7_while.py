...
while treeHit < 10 : # 변수가 10미만이면 반복하고 아니면 반복종료 한다는 뜻
    treeHit = treeHit + 1  # py 증감연산자 없다. java/js 증감식 : treeHit++
    print(f'나무를{treeHit}번 찍었습니다.') # f포메팅 : f'문자열{코드}문자열'
    if treeHit == 10 :
        print('나무 넘어갑니다.')

# while , if 사용할때 주의할점 : { } 없고 들여쓰기를 이용한 실행문들을 제어문의 포함하자.

# (2) while 문 만들기
number = 0              # 변수
while number != 4 :     # 변수가 4가 아니면 반복하고 4이면 반복 종료 하겠다 뜻
...
while True : # 무한루프 , java : while(true){} , 파이썬에서는 True , False 첫글자를 대문자
    ...
    # int(자료) : 해당 자료를 정수 타입 반환 함수 , vs Integer.parseInt() vs JS : parseInt()
    if money == 300 :
        print('커피를 줍니다')
        coffee = coffee - 1
    elif money > 300 :
        print('거스름돈 %d를 주고 커피를 줍니다.' %(money - 300)) # f 포메팅 : f'문자열(코드) 문자열'
        coffee -= 1
    else : # 아니면
    ...
    if coffee == 0 : # 만약에 커피가 0개 이면
        print(f'커피가 떨어졌습니다. 판매를 중지합니다.')
        break # 가장 가까운 반복문을 강제로 종료 한다.

# (4) continue 키워드 : 가장 가까운 반복문으로 강제 이동
a = 0                   # (1 초기값) 반복 변수
while a < 10 :          # (2 조건문) 반복 조건
    a += 1              # (3 증감식) a의 값을 1 증가
    if a % 2 == 0 :     # a의 값이 짝수 이면
        continue        # while 문으로 이동한다, 아래 코드는 실행되지 않는다.
    print(a)            # 짝수는 출력되지 않고 , 홀수만 출력된다 1 3 5 7 9

# (5) 무한루프 : 저간이 항상 True 이므로 while 안에 있는 실행문은 무한 수행하게 된다. 주로 break를 이용한 종료 조건을 만든다
'''
    while True : 
'''
