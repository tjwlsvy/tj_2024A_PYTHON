...
# [1] # urllib.request : URL 요청에 관한 기능을 제공하는 라이브러리
    # urllib.request.Request(url) : 지정한 url에 대한 요청 객체를 반환
    # urllib.request.urlopen(요청객체) : 지정한 요청 객체를 실행하고 응답 객체를 반환
    # urllib.parse.quote(문자열) : 지정한 문자열을 HTTP 바이트로 반환
...
응답객체 = urllib.request.urlopen(요청객체) # urlopen 메소드를 이용한 url에 대한 요청을 실행하고 응답을 반환 함수
print(응답객체) # <http.client.HTTPResponse object at 0x00000297E62D40A0>
